...
func TestMandatoryOnboardingAndSessionRevocation(t *testing.T) {
	...
	authRequest(t, s, "/api/auth/mfa/setup", token, nil, s.authMiddleware(s.handleMFASetup), 409)
	authRequest(t, s, "/api/auth/mfa/enable", token, map[string]string{"secret": secret, "code": code}, s.authMiddleware(s.handleMFAEnable), 409)
