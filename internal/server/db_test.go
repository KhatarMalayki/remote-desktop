...
func TestAgentRegistrationDoesNotAutoUpdate(t *testing.T) {
	...
	s := &Server{cfg: Config{Version: "0.2.14", APIKey: "test-key"}, db: db}
	s.hub = NewHub(db)
...
func TestQueueAgentUpdateOnlyForOnlineOutdatedDevice(t *testing.T) {
	...
	case raw := <-client.Send:
		if !strings.Contains(string(raw), `"action":"upgrade"`) || !strings.Contains(string(raw), `"version":"0.2.14"`) {
